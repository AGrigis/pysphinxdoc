...
def get_subsections(srcdir, examples_dir, sortkey):
    ...
    subfolders = [subfolder for subfolder in os.listdir(examples_dir)
                  if os.path.exists(os.path.join(examples_dir, subfolder,
                                    'README.txt'))]
...
def generate_gallery_rst(app):
    ...
    for examples_dir, gallery_dir in workdirs:
        ...
        with codecs.open(os.path.join(gallery_dir, 'gallery.rst'), 'w',
...
def sumarize_failing_examples(app, exception):
    """Collects the list of falling examples during build and prints them with
    the traceback
